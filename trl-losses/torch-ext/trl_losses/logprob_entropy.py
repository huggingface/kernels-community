...
def _forward_kernel(
    ...
    logits_row_ptr = (
        logits_ptr + batch * logits_batch_stride + row_in_batch * logits_row_stride
    )
    index = tl.load(
        index_ptr + batch * index_batch_stride + row_in_batch * index_row_stride
    )
    ...
    for start in range(0, n_cols, BLOCK_SIZE):
        ...
        mask = offsets < n_cols
        logits = tl.load(logits_row_ptr + offsets, mask=mask, other=-float("inf")).to(
            tl.float32
        )
        ...
        weighted_sum = weighted_sum * correction + tl.sum(
            probabilities * tl.where(mask, logits, 0.0), axis=0
        )
        row_max = new_max

    log_z = row_max + tl.log(denominator)
    expected_logit = weighted_sum / denominator
    target_logit = tl.load(logits_row_ptr + index).to(tl.float32)
...
def _backward_kernel(
    ...
    grad_logits_row_ptr = (
        grad_logits_ptr
        + batch * grad_logits_batch_stride
        + row_in_batch * grad_logits_row_stride
    )
    logits = tl.load(logits_row_ptr + offsets, mask=mask).to(tl.float32)
    index = tl.load(
        index_ptr + batch * index_batch_stride + row_in_batch * index_row_stride
    )
    ...
    tl.store(grad_logits_row_ptr + offsets, grad, mask=mask)
...
class _LogProbEntropyFunction(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, logits: torch.Tensor, index: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        batch_size, rows_per_batch, logits_batch_stride, logits_row_stride = _layout(
            logits
        )
        ...
        ctx.save_for_backward(logits, index, log_z, expected_logit)
    ...
    def backward(
        ...
        logits, index, log_z, expected_logit = ctx.saved_tensors
...
def selective_log_softmax_and_entropy(
    ...
    if not 1 <= logits.ndim <= 3:
        raise ValueError(
            "logits must have shape [vocab], [tokens, vocab], or [batch, tokens, vocab]"
        )
    ...
    return _LogProbEntropyFunction.apply(logits, index)
