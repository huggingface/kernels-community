...
def _compute_source_fingerprint() -> str:
    ...
    # the top-level package keeps the fingerprint stable regardless of where
    # inside the package this file lives. The package is looked up through
    # `__package__` rather than by the literal name `quack`, because the
    # `kernels` loader registers Hub kernels under a randomized module name.
    _quack = importlib.import_module(__package__.partition(".")[0])
...
def jit_cache(fn):
    ...
    * **Slow path (cold cache).** The actual ``fn(*args, **kwargs)`` compile
    ...
    def wrapper(*args, **kwargs):
        ...
        from ..cache import async_compile as _async
