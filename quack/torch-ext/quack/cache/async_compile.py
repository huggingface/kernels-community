...
def _make_executor(jobs: int) -> ProcessPoolExecutor:
    ...
    if start_method == "forkserver":
        # Derived from `__package__` rather than hard-coded: the `kernels`
        # loader registers Hub kernels under a randomized module name.
        ctx.set_forkserver_preload([f"{__package__}._pool_preload"])
