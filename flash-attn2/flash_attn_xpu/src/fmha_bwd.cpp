...
template <typename Policy, int IsVarLen>
void dispatch_bwd_causal_local(sycl::queue& queue, BwdCutlassType cuType,
    ...
    if (is_causal && is_local) {
        bwd_policy_dispatch<Policy, 1, 1, IsVarLen>(queue, cuType, args);
    } else if (is_causal) {
        bwd_policy_dispatch<Policy, 1, 0, IsVarLen>(queue, cuType, args);
    } else if (is_local) {
        bwd_policy_dispatch<Policy, 0, 1, IsVarLen>(queue, cuType, args);
    } else {
        bwd_policy_dispatch<Policy, 0, 0, IsVarLen>(queue, cuType, args);
    }
}
...
void dispatch_bwd_causal_local(sycl::queue& queue, BwdCutlassType cuType,
                               const fmha_bwd_args_t& args,
                               bool is_causal, bool is_local, bool is_varlen) {
    if (is_varlen) {
        dispatch_bwd_causal_local<Policy, 1>(queue, cuType, args, is_causal, is_local);
    } else {
        dispatch_bwd_causal_local<Policy, 0>(queue, cuType, args, is_causal, is_local);
    }
}
...
void dispatch_bwd_by_head(sycl::queue& queue, BwdCutlassType cuType,
                          ...
                          bool is_causal, bool is_local, bool is_varlen) {
    if      (head_size <=  32) dispatch_bwd_causal_local<bwd_policy_head32> (queue, cuType, args, is_causal, is_local, is_varlen);
    else if (head_size <=  64) dispatch_bwd_causal_local<bwd_policy_head64> (queue, cuType, args, is_causal, is_local, is_varlen);
    else if (head_size <=  96) dispatch_bwd_causal_local<bwd_policy_head96> (queue, cuType, args, is_causal, is_local, is_varlen);
    else if (head_size <= 128) dispatch_bwd_causal_local<bwd_policy_head128>(queue, cuType, args, is_causal, is_local, is_varlen);
    else if (head_size <= 160) dispatch_bwd_causal_local<bwd_policy_head160>(queue, cuType, args, is_causal, is_local, is_varlen);
    else if (head_size <= 192) dispatch_bwd_causal_local<bwd_policy_head192>(queue, cuType, args, is_causal, is_local, is_varlen);
    else if (head_size <= 256) dispatch_bwd_causal_local<bwd_policy_head256>(queue, cuType, args, is_causal, is_local, is_varlen);
    else if (head_size == 512) dispatch_bwd_causal_local<bwd_policy_head512>(queue, cuType, args, is_causal, is_local, is_varlen);
...
void cutlass_fmha_bwd_fix_impl(
    ...
    dispatch_bwd_by_head(queue, cuType, args, args.head_size, is_causal, is_local, false);
}
...
void cutlass_fmha_bwd_varlen_impl(
    ...
    const int kBlockM_bwd = bwd_block_m_for_head(head_size);
    const int kBlockN_bwd = bwd_block_n_for_head(head_size);
    const int seqlen_q_rounded = round_up(max_seqlen_q, kBlockM_bwd);
    const int seqlen_k_rounded = round_up(max_seqlen_k, kBlockN_bwd);
    ...
    if (!deterministic) {
        ...
        const int num_compute_units = static_cast<int>(queue.get_device().get_info<sycl::info::device::max_compute_units>());
        nsplits = std::max((num_compute_units + batch_size * num_heads_q - 1) / (batch_size * num_heads_q), 1);
        const int max_n_blocks = std::max((max_seqlen_k + 31) / 32, 1);
        nsplits = std::min(nsplits, max_n_blocks);
        dq_accum = at::zeros({nsplits, batch_size, num_heads_q, seqlen_q_rounded, head_size},
                             q.options().dtype(at::kFloat));
        dq_accum_split_stride = batch_size * seqlen_q_rounded * num_heads_q * head_size;
    ...
    const BwdCutlassType cuType = aten_to_Bwd_Cutlass_dtype(q);
    dispatch_bwd_by_head(queue, cuType, args, args.head_size, is_causal, is_local, true);
}
