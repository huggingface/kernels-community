...
def cutlass_fp8_gemm_helper(m: int,
    ...
    out = vllm_quantization.cutlass_scaled_mm(a, b, scale_a, scale_b, out_dtype, bias)
...
def test_cutlass_int8_azp_bias_fold(m: int, n: int, k: int,
    ...
    out = vllm_quantization.cutlass_scaled_mm(aq_i8,
                                bq_i8,
                                scale_a,
                                scale_b,
                                out_dtype=out_dtype,
...
def test_cutlass_int8_azp(m: int, n: int, k: int, out_dtype: torch.dtype,
    ...
    if azp_per_token:
        out = vllm_quantization.cutlass_scaled_mm_azp(aq_i8, bq_i8, scale_a, scale_b,
...
def test_cutlass_subset():
    ...
    out = vllm_quantization.cutlass_scaled_mm(a,
                                b,
                                scale_a,
                                scale_b,
                                out_dtype=torch.bfloat16)
...
class CutlassLayer(torch.nn.Module):
    ...
    def forward(self, a):
        return vllm_quantization.cutlass_scaled_mm(a, self.b, self.scale_a, self.scale_b,
                                     self.out_dtype)
