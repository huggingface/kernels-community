...
bnb = get_kernel("kernels-community/quantization-bitsandbytes")
device = torch.device("cpu")  # BitsAndBytes CPU kernel

# 4-bit quantized GEMM forward pass
# Useful for memory-efficient inference with quantized weights
batch_size = 2
seq_len = 16
in_features = 64
out_features = 128
blocksize = 64  # quantization block size (typically 64 or 128)
...
# In practice, these would come from a quantized model
...
# quant_type: 0 for fp4, 1 for nf4
quant_type = 1  # nf4 quantization

# Run 4-bit GEMM forward
output = bnb.gemm_4bit_forward(input_tensor, weight, absmax, blocksize, quant_type)
