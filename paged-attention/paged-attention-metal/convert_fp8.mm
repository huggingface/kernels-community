...
static std::string getConvertKernelName(torch::ScalarType src_dtype,
...
void convert_fp8(torch::Tensor &dst_cache, torch::Tensor &src_cache,
  ...
  std::string kernel_name =
      getConvertKernelName(src_cache.scalar_type(), dst_cache.scalar_type());
  ...
  @autoreleasepool {
    ...
    dispatch_sync(q, ^{
      ...
      id<MTLBuffer> scaleBuf =
          [device newBufferWithBytes:&scale_f32
                              length:sizeof(float)
                             options:MTLResourceStorageModeShared];
      [enc setBuffer:scaleBuf offset:0 atIndex:2];

      // Set num_elements parameter
      id<MTLBuffer> numElementsBuf =
          [device newBufferWithBytes:&num_elements
                              length:sizeof(uint32_t)
                             options:MTLResourceStorageModeShared];
      [enc setBuffer:numElementsBuf offset:0 atIndex:3];

      // Dispatch threads
      const uint32_t threads_per_threadgroup =
          std::min<uint32_t>(1024, num_elements);
      const uint32_t threadgroups =
          (num_elements + threads_per_threadgroup - 1) /
          threads_per_threadgroup;

      MTLSize threadsPerThreadgroup =
          MTLSizeMake(threads_per_threadgroup, 1, 1);
