...
VLLM_ENABLE_MOE_ALIGN_BLOCK_SIZE_TRITON: bool = bool(
    int(os.getenv("VLLM_ENABLE_MOE_ALIGN_BLOCK_SIZE_TRITON", "0"))
)
...
def invoke_fused_moe_kernel(
    ...
    if use_fp8_w8a8:
        ...
        assert (
            block_shape is None
            or triton.cdiv(B.shape[-2], block_shape[0]) == B_scale.shape[-2]
        )
        assert (
            block_shape is None
            or triton.cdiv(B.shape[-1], block_shape[1]) == B_scale.shape[-1]
        )
...
def fused_topk(
    ...
    topk_weights, topk_ids = topk_func(
        topk_weights, topk_ids, token_expert_indicies, gating_output_float, renormalize
    )
...
def torch_vllm_inplace_fused_experts(**kwargs) -> torch.Tensor:
    ops.inplace_fused_experts(**kwargs)
    hidden_states = kwargs["hidden_states"]
    return hidden_states


def torch_vllm_outplace_fused_experts(**kwargs) -> torch.Tensor:
    return ops.outplace_fused_experts(**kwargs)
