...
// NOTE: repackaging change — upstream includes <torch/python.h>, but that pulls
// pybind11 which fails to compile under kernel-builder's -DPy_LIMITED_API. The
// kernels here use the torch.library C++ API (no pybind), so <torch/all.h> is
// sufficient.
#include <torch/all.h>
