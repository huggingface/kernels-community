...
void count_cumsum_cuda(const torch::Tensor &x, torch::Tensor &count_output,
  ...
  cudaStream_t stream =
      at::cuda::getCurrentCUDAStream(x.device().index()).stream();

  CHECK_CUDA_TENSOR(x);
  CHECK_CUDA_TENSOR(count_output);
  if (do_cumsum) {
    CHECK_CUDA_TENSOR(cumsum_output);
  }

  const uint32_t E = count_output.size(0);

  TORCH_CHECK(E <= MAX_ALLOWED_E, "E must be <= ", MAX_ALLOWED_E);
  TORCH_CHECK(E % 4 == 0, "E must be divisible by 4");
  ...
  DISPATCH_INT_KERNEL(
      x.scalar_type(), "count_cumsum_cuda_kernel", scalar_t, ([&] {
        // Use actual E instead of MAX_ALLOWED_E to avoid exceeding shared memory limits on pre-H100 GPUs.
        ...
        cudaLaunchKernelEx(
            ...
            (do_cumsum ? count_cumsum_cuda_kernel<scalar_t, true>
                       : count_cumsum_cuda_kernel<scalar_t, false>),
            x.data_ptr<scalar_t>(),
            count_output.data_ptr<int32_t>(),
            do_cumsum ? cumsum_output.data_ptr<int32_t>() : nullptr, N, E);
      }));
}
