...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> mha_bwd(
    ...
    if (num_heads_k != num_heads) {  // MQA / GQA
        if (!is_varlen) {
            ...
            dv_accum = torch::stable::new_zeros(q, {batch_size, num_heads_k, seqlen_k_rounded * head_size_v_rounded}, std::make_optional(torch::headeronly::ScalarType::Float));
        } else {
            ...
            dv_accum = torch::stable::new_zeros(q, {num_heads_k, total_k_padded_rounded, head_size_v_rounded}, std::make_optional(torch::headeronly::ScalarType::Float));
        }
    }
...
void boxed_mha_fwd(
    ...
    auto softmax_scale = torch::stable::detail::to<std::optional<double>>(stack[23]);
    ...
    auto attention_chunk = torch::stable::detail::to<int64_t>(stack[27]);
    auto softcap = torch::stable::detail::to<double>(stack[28]);
    auto is_rotary_interleaved = torch::stable::detail::to<bool>(stack[29]);
    auto scheduler_metadata = torch::stable::detail::to<std::optional<Tensor>>(stack[30]);
    auto num_splits = torch::stable::detail::to<int64_t>(stack[31]);
    auto pack_gqa = torch::stable::detail::to<std::optional<bool>>(stack[32]);
    auto sm_margin = torch::stable::detail::to<int64_t>(stack[33]);
    auto sinks = torch::stable::detail::to<std::optional<Tensor>>(stack[34]);

    auto [out_, softmax_lse, out_accum, softmax_lse_accum] = mha_fwd(q, k, v, k_new, v_new, q_v, out, cu_seqlens_q, cu_seqlens_k, cu_seqlens_k_new, seqused_q, seqused_k, max_seqlen_q, max_seqlen_k, page_table, kv_batch_idx, leftpad_k, rotary_cos, rotary_sin, seqlens_rotary, q_descale, k_descale, v_descale, softmax_scale, is_causal, window_size_left, window_size_right, attention_chunk, softcap, is_rotary_interleaved, scheduler_metadata, num_splits, pack_gqa, sm_margin, sinks);
