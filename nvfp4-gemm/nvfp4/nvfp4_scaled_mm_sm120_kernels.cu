// Adapted from vLLM: csrc/libtorch_stable/quantization/fp4/nvfp4_scaled_mm_sm120_kernels.cu
// https://github.com/vllm-project/vllm (Apache-2.0). Local changes: include path;
// cutlass_fp4_gemm_dispatch given internal linkage.
/*
...
typename Gemm::Arguments args_from_options(nvfp4_tensor& D,
                                           nvfp4_tensor const& A,
                                           nvfp4_tensor const& B,
                                           nvfp4_tensor const& A_sf,
                                           nvfp4_tensor const& B_sf,
                                           nvfp4_tensor const& alpha, int M,
                                           int N, int K) {
...
void runGemm(nvfp4_tensor& D, nvfp4_tensor const& A, nvfp4_tensor const& B,
             ...
             nvfp4_tensor const& alpha, int M, int N, int K,
             cudaStream_t stream) {
  Gemm gemm;
...
void cutlass_scaled_fp4_mm_sm120a(nvfp4_tensor& D, nvfp4_tensor const& A,
  ...
  if (out_dtype == ScalarType::BFloat16) {
    return cutlass_fp4_gemm_dispatch<cutlass::bfloat16_t>(
        D, A, B, A_sf, B_sf, alpha, m, n, k, stream);
  } else if (out_dtype == ScalarType::Half) {
