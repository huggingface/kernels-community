...
void {kernel_name}(
  ...
  auto run = [&](auto kIsDeterministic) {{
    ...
    using Kernel = natten::cuda::fmha_blackwell::KernelBackward<
      {dtype}, GemmShape, cutlass::fmha::collective::NoMask, /* kIsVarlen= */ false, IsDeterministic>;
    using KernelWithResidualMask = natten::cuda::fmha_blackwell::KernelBackward<
      {dtype}, GemmShape, cutlass::fmha::collective::ResidualMaskForBackward, /* kIsVarlen= */ false, IsDeterministic>;
    using KernelWithCausalMask = natten::cuda::fmha_blackwell::KernelBackward<
      {dtype}, GemmShape, cutlass::fmha::collective::CausalForBackwardMask<true>, /* kIsVarlen= */ false, IsDeterministic>;
...
def write_combined_source_file(path, filename, headers, kernels):
    ...
    source_head += ["#include <natten/cuda/fmha_blackwell/fmha_backward.cuh>\n"]
...
class DTypeDispatcher:
    def __init__(self):
        self.dtypes = []
        self.name = "DISPATCH_BLACKWELL_FMHA_BACKWARD"
    ...
    def get_dispatcher(self):
        ...
        dispatcher_str += "    else { \\\n"
        dispatcher_str += (
            '      throw std::runtime_error("Blackwell FMHA backward kernel dispatch failed! '
...
class HeadDimDispatcher:
    ...
    def get_dispatcher(self):
        ...
        dispatcher_str += (
            ...
            '"Blackwell FMHA backward kernel dispatch failed! '
...
class TileSizeDispatcher:
    def __init__(
        ...
        self.name = f"DISPATCH_BLACKWELL_FMHA_BACKWARD_{self.dtype.short_name}_headdim{head_dim}"
    ...
    def get_kernel_instance(self, q_tile_size, kv_tile_size):
        ...
        supported_gemm_shapes = SUPPORTED_GEMM_SHAPES_BACKWARD
...
def generate_blackwell_fmha_kernels(path, num_splits=2):
    ...
    for dtype in SUPPORTED_DTYPES:
        ...
        for head_dim in HEAD_DIMS:
            ...
            for tile_size in TILE_SIZES:
                ...
                kernels.append(
                    tile_size_dispatcher.get_kernel_instance(tile_size[0], tile_size[1])
                )
    ...
    path_to_sources = f"{path}/autogen/src/cuda/blackwell_fmha_bwd/"
    ...
    cuda_headers = [
        ...
        "natten/natten.h",
        "natten/helpers.h",
        "natten/cuda/fmha_blackwell/fmha_backward.cuh",
    ]
    write_header_file(
