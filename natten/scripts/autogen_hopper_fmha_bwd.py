...
def write_combined_source_file(path, filename, headers, kernels):
    ...
    source_head += ["#include <natten/cuda/fmha_hopper/fmha_backward.cuh>\n"]
...
class DTypeDispatcher:
    def __init__(self):
        self.dtypes = []
        self.name = "DISPATCH_HOPPER_FMHA_BACKWARD"
...
class ConfigDispatcher:
    def __init__(
        ...
        self.name = (
            f"DISPATCH_HOPPER_FMHA_BACKWARD_{self.dtype.short_name}_headdim{head_dim}"
        )
...
def generate_hopper_fmha_kernels(path, num_splits=2):
    ...
    path_to_sources = f"{path}/autogen/src/cuda/hopper_fmha_bwd/"
    rel_header = "natten_autogen/cuda/hopper_fmha_bwd/"
    ...
    cuda_headers = [
        ...
        "natten/natten.h",
        "natten/helpers.h",
        "natten/cuda/fmha_hopper/fmha_backward.cuh",
    ]
    write_header_file(
