...
class BlackwellFnaInstance:
    ...
    def get_name(self) -> str:
        name = f"blackwell_fna{self.na_dim}d_backward"
...
def write_combined_source_file(path, filename, headers, kernels):
    ...
    source_head += ["#include <natten/cuda/fna_blackwell/fna_backward.cuh>\n"]
...
class NaDimDispatcher:
    def __init__(self):
        self.name = "DISPATCH_BLACKWELL_FNA_BACKWARD"
        self.dims = []
    ...
    def get_dispatcher(self):
        ...
        dispatcher_str += "    else { \\\n"
        dispatcher_str += (
            '      throw std::runtime_error("Blackwell FNA backward kernel dispatch failed! '
...
class DTypeDispatcher:
    def __init__(self, na_dim: int):
        ...
        self.na_dim = na_dim
        self.name = f"DISPATCH_BLACKWELL_FNA_BACKWARD_{self.na_dim}D"
    ...
    def get_dispatcher(self):
        ...
        dispatcher_str += "    else { \\\n"
        dispatcher_str += (
            '      throw std::runtime_error("Blackwell FNA-'
...
class HeadDimDispatcher:
    def __init__(self, na_dim: int, dtype: DataType):
        ...
        self.dtype = dtype
        self.name = (
            f"DISPATCH_BLACKWELL_FNA_BACKWARD_{self.na_dim}D_{self.dtype.short_name}"
        )
    ...
    def get_dispatcher(self):
        ...
        dispatcher_str += (
            ...
            '"Blackwell FNA backward kernel dispatch failed! '
...
class CausalMaskDispatcher:
    def __init__(
        ...
        self.name = f"DISPATCH_BLACKWELL_FNA_BACKWARD_{self.na_dim}D_{self.dtype.short_name}_headdim{head_dim}"
...
class TileShapeDispatcher:
    ...
    def get_kernel_instance(self, q_tile_shape, kv_tile_shape):
        ...
        assert gemm_shape in SUPPORTED_GEMM_SHAPES_BACKWARD
    ...
    def get_dispatcher(self):
        ...
        dispatcher_str += (
            '      throw std::runtime_error("Blackwell FNA-'
            + f"{self.na_dim}D backward kernel dispatch failed! "
            f"It got invalid Q tile and KV tile combination "
...
def generate_blackwell_fna_kernels(path, num_splits=2):
    ...
    path_to_sources = f"{path}/autogen/src/cuda/blackwell_fna_bwd/"
    rel_header = "natten_autogen/cuda/blackwell_fna_bwd/"
    ...
    cuda_headers = [
        ...
        "natten/natten.h",
        "natten/helpers.h",
        "natten/cuda/fna_blackwell/fna_backward.cuh",
    ]
