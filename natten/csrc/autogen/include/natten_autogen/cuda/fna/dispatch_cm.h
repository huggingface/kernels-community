...
namespace cuda { 
namespace fna { 
#define DISPATCH_FNA_FORWARD_1D_SM50_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D, SM50, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_1D_SM50_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D, SM50, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_1D_SM70_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D, SM70, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_1D_SM70_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D, SM70, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_1D_SM75_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D, SM75, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_1D_SM75_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D, SM75, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_1D_SM80_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D, SM80, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_1D_SM80_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D, SM80, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_1D_SM80_bfloat16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D, SM80, bfloat16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_2D_SM50_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D, SM50, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_2D_SM50_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D, SM50, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_2D_SM70_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D, SM70, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_2D_SM70_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D, SM70, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_2D_SM75_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D, SM75, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_2D_SM75_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D, SM75, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_2D_SM80_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D, SM80, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_2D_SM80_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D, SM80, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_2D_SM80_bfloat16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D, SM80, bfloat16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_3D_SM50_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D, SM50, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_3D_SM50_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D, SM50, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_3D_SM70_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D, SM70, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_3D_SM70_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D, SM70, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_3D_SM75_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D, SM75, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_3D_SM75_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D, SM75, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_3D_SM80_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D, SM80, float32) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_3D_SM80_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D, SM80, float16) got invalid causal mask!"); \
    } \
}();

#define DISPATCH_FNA_FORWARD_3D_SM80_bfloat16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D, SM80, bfloat16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_1D_SM50_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D-backward, SM50, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_1D_SM50_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D-backward, SM50, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_1D_SM70_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D-backward, SM70, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_1D_SM70_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D-backward, SM70, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_1D_SM75_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D-backward, SM75, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_1D_SM75_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D-backward, SM75, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_1D_SM80_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D-backward, SM80, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_1D_SM80_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D-backward, SM80, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_1D_SM80_bfloat16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-1D-backward, SM80, bfloat16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_2D_SM50_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D-backward, SM50, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_2D_SM50_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D-backward, SM50, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_2D_SM70_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D-backward, SM70, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_2D_SM70_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D-backward, SM70, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_2D_SM75_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D-backward, SM75, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_2D_SM75_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D-backward, SM75, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_2D_SM80_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D-backward, SM80, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_2D_SM80_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D-backward, SM80, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_2D_SM80_bfloat16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-2D-backward, SM80, bfloat16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_3D_SM50_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D-backward, SM50, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_3D_SM50_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D-backward, SM50, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_3D_SM70_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D-backward, SM70, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_3D_SM70_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D-backward, SM70, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_3D_SM75_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D-backward, SM75, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_3D_SM75_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D-backward, SM75, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_3D_SM80_float32(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D-backward, SM80, float32) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_3D_SM80_float16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D-backward, SM80, float16) got invalid causal mask!"); \
    } \
}();
...
#define DISPATCH_FNA_BACKWARD_3D_SM80_bfloat16(is_causal, cb) \
  [&] { \
    if (!std::get<0>(is_causal) && !std::get<1>(is_causal) && !std::get<2>(is_causal)) { \
      ...
      throw std::runtime_error("NATTEN FNA kernel dispatch failed! Causal mask dispatcher (FNA-3D-backward, SM80, bfloat16) got invalid causal mask!"); \
    } \
}();
