...
#define DISPATCH_HOPPER_FNA_BACKWARD_1D_float16(dim, is_causal, q_tile_shape, kv_tile_shape, ...) \
  [&] { \
    if (dim == 32) { \
      ...
      throw std::runtime_error("Hopper FNA backward kernel dispatch failed! It does not support head dim " + std::to_string(dim) + " for float16."); \
    } \
}();
...
#define DISPATCH_HOPPER_FNA_BACKWARD_1D_bfloat16(dim, is_causal, q_tile_shape, kv_tile_shape, ...) \
  [&] { \
    if (dim == 32) { \
      ...
      throw std::runtime_error("Hopper FNA backward kernel dispatch failed! It does not support head dim " + std::to_string(dim) + " for bfloat16."); \
    } \
}();
