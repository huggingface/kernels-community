  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_1D_float16_headdim32_causal1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-1D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_1D_float16_headdim64_causal1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-1D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_1D_float16_headdim128_causal1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-1D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 128) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_1D_float16_headdim256_causal1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-1D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 256) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_1D_bfloat16_headdim32_causal1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-1D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_1D_bfloat16_headdim64_causal1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-1D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_1D_bfloat16_headdim128_causal1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-1D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 128) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_1D_bfloat16_headdim256_causal1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-1D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 256) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_2D_float16_headdim32_causal1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-2D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_2D_float16_headdim64_causal1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-2D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_2D_float16_headdim128_causal1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-2D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 128) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_2D_float16_headdim256_causal1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-2D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 256) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_2D_bfloat16_headdim32_causal1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-2D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_2D_bfloat16_headdim64_causal1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-2D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_2D_bfloat16_headdim128_causal1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-2D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 128) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_2D_bfloat16_headdim256_causal1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-2D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 256) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_3D_float16_headdim32_causal1x1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-3D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_3D_float16_headdim64_causal1x1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-3D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_3D_float16_headdim128_causal1x1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-3D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 128) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_3D_float16_headdim256_causal1x1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-3D forward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 256) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_3D_bfloat16_headdim32_causal1x1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-3D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_3D_bfloat16_headdim64_causal1x1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-3D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_3D_bfloat16_headdim128_causal1x1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-3D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 128) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_HOPPER_FNA_FORWARD_3D_bfloat16_headdim256_causal1x1x1(q_tile_shape, kv_tile_shape, kernel_type, __VA_ARGS__); \
          ...
          throw std::runtime_error("Hopper FNA-3D forward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 256) got invalid causal mask!"); \
    } \
}();
