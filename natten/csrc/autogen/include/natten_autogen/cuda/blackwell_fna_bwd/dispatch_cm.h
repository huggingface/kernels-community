  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_1D_float16_headdim32_causal1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-1D backward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_1D_float16_headdim64_causal1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-1D backward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_1D_float16_headdim128_causal1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-1D backward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 128) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_1D_bfloat16_headdim32_causal1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-1D backward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_1D_bfloat16_headdim64_causal1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-1D backward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_1D_bfloat16_headdim128_causal1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-1D backward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 128) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_2D_float16_headdim32_causal1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-2D backward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_2D_float16_headdim64_causal1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-2D backward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_2D_float16_headdim128_causal1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-2D backward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 128) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_2D_bfloat16_headdim32_causal1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-2D backward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_2D_bfloat16_headdim64_causal1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-2D backward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_2D_bfloat16_headdim128_causal1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-2D backward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 128) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_3D_float16_headdim32_causal1x1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-3D backward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_3D_float16_headdim64_causal1x1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-3D backward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_3D_float16_headdim128_causal1x1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-3D backward kernel dispatch failed! Causal mask dispatcher (float16, head_dim 128) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_3D_bfloat16_headdim32_causal1x1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-3D backward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 32) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_3D_bfloat16_headdim64_causal1x1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-3D backward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 64) got invalid causal mask!"); \
    } \
}();
  ...
  [&] { \
    if (not cute::get<0>(is_causal) && not cute::get<1>(is_causal) && not cute::get<2>(is_causal)) { \
      ...
      DISPATCH_BLACKWELL_FNA_BACKWARD_3D_bfloat16_headdim128_causal1x1x1(q_tile_shape, kv_tile_shape, __VA_ARGS__); \
          ...
          throw std::runtime_error("Blackwell FNA-3D backward kernel dispatch failed! Causal mask dispatcher (bfloat16, head_dim 128) got invalid causal mask!"); \
    } \
}();
